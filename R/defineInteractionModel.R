...
#'
#' @param interaction `character`; Formula specifying whether
#' to fit a `Hardcore`,
#' `Strauss`, `Fiksel` or `StraussHard` process to the data
#' @param cellspacing `numeric` how much spacing should be
#' accounted for in the interaction process due to the cell body.
#' If this is not provided, the cell spacing parameter is estimated
#' from the data as the minimum nearest neighbour distance
#' divided by $n(n+1)$ as done in `spatstat.model::Hardcore`.
#' In the case of `StraussHard`, only the Strauss interaction radius
#' can be user-provided, the Hardcore interaction is always estimated
#' from the data.
...
#' )
#'
defineInteractionModel <- function(
    interaction,
    cellspacing,
    response,
    data
    ...
    if (is.null(interaction)) {
        ...
        message("You have specified a Poisson process -
            make sure that the assumption of physical overlap
            is justified in your data")
        ...
        pg <- spatstat.model::profilepl(rs, spatstat.model::StraussHard,
            data[[response]], verbose = FALSE, fast = TRUE
        )
        ...
        pg <- spatstat.model::profilepl(rs, spatstat.model::Fiksel,
            data[[response]],
            verbose = FALSE, fast = TRUE
        )
        interactionModel <- pg$fit$interaction
    } else {
        warning("Interaction model ", interaction, " is not implemented")
    }
